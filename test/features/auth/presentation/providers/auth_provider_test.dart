...
void main() {
  ...
  group('AuthProvider estado inicial', () {
    ...
    test(
      'deve restaurar sessão quando há usuário autenticado no Supabase',
      () async {
        when(() => mockRepository.usuarioAtual).thenReturn(_fakeUser());
        when(
          () => mockRepository.buscarPerfil(any()),
  ...
  group('AuthProvider.login', () {
    test(
      'deve atualizar estado com usuario quando login é bem-sucedido',
      () async {
        when(() => mockRepository.usuarioAtual).thenReturn(null);
        when(
          () => mockRepository.authStateChanges,
