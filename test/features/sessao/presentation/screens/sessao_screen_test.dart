...
void main() {
  ...
  Widget buildWidget() {
    final router = GoRouter(
      ...
      routes: [
        ...
        GoRoute(path: '/gestor/sessao/abrir', builder: (ctx, state) => const Scaffold()),
      ],
    );
  ...
  group('SessaoScreen', () {
    testWidgets('deve exibir mensagem quando não há sessão aberta', (tester) async {
    ...
    testWidgets('deve exibir botão de abrir sessão quando não há sessão', (tester) async {
      when(() => mockRepository.buscarSessaoAberta())
          .thenAnswer((_) async => null);
    ...
    testWidgets('deve exibir botão de encerrar quando há sessão aberta', (tester) async {
      when(() => mockRepository.buscarSessaoAberta())
    ...
    testWidgets('deve exibir médiuns/entidades da sessão quando aberta', (tester) async {
      when(() => mockRepository.buscarSessaoAberta())
          .thenAnswer((_) async => sessaoFake);
      when(() => mockRepository.listarMediumEntidadesDaSessao(any()))
          .thenAnswer((_) async => [mediumEntidadeFake]);
      ...
      expect(
        find.text('${mediumEntidadeFake.mediumNome} — ${mediumEntidadeFake.entidadeNome}'),
        findsOneWidget,
      );
    });
  });
